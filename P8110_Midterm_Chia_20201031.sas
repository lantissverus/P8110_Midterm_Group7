...
we decided to create a new variable to combine them together, 
and examine the proportional hazard assumption again.*/

data midterm_ses;
set midterm_h1;
if ses_class in (1, 5) then ses_new = 1;
else if ses_class = 2  then ses_new = 2;
else if ses_class = 3 then ses_new = 3;
else if ses_class = 4 then ses_new = 4;
run;
...
*******************************************
Once we combine ses class 1 and 5 into a new variable group, the new variable does not
